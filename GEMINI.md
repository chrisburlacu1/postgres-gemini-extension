...
- **Modularity:** When adding new tools, place them in the appropriate category within the `src/tools/` directory (or create a new category file if necessary) and ensure they are exported and registered in `src/tools/index.ts`.
- **Database Interaction:** Use connection pooling (via the `pg` library) rather than opening a new client for every request. Be extremely careful to prevent SQL injection vulnerabilities; use parameterized queries whenever accepting user input.
- **Error Handling:** Provide clear, actionable error messages back to the client if a query or operation fails.

## Key Features & Tools
The extension provides several tools categorized as follows:
- **Query:** `execute_query`
- **Schema Discovery:** `list_databases`, `list_tables`, `list_views`, `describe_table`, `search_schema`
